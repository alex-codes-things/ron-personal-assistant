...
- Current Ron Face version: 0.1.11
...
`FaceAnimator`. `RonTabletPager` owns the native two-page slide transition and
its corner buttons. `SignalServer` receives only compact state and speech-level
...
The second page is intentionally blank for now and has a mirrored return button
in its top-left corner. Android's Back action also returns to the face page.
